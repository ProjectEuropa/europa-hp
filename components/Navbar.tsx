				...
				<div className="container mx-auto px-4 md:px-8">
					...
					<div className="flex flex-row items-center justify-between md:flex-col md:items-center md:justify-center animate-header-appear">
						...
						<div className="flex flex-col items-start md:items-center text-left md:text-center w-auto md:w-full mb-0 md:mb-2">
							...
							<div
								className="font-sans text-xs md:text-base font-light text-foreground tracking-[1px] md:tracking-[2px] uppercase opacity-80 h-auto md:h-5 leading-normal md:leading-5 animate-typing overflow-hidden whitespace-nowrap block"
						...
						<nav className="hidden md:flex justify-center mt-2 space-x-4">
							{navLinks.map((link, index) => (
								<Link
									key={link.href}
									href={link.href}
									className={`relative group flex flex-col items-center px-4 py-2 transition-all duration-300 ${
										pathname === link.href
											? "text-cyber-blue"
								...
								>
									...
									<span className="text-[9px] tracking-[1px] opacity-70 group-hover:opacity-100">
										{link.jp}
									</span>

									{/* Brackets Effect */}
									<span
										className={`absolute left-0 top-2 opacity-0 transition-opacity duration-300 text-cyber-blue ${pathname === link.href ? "opacity-100" : "group-hover:opacity-100"}`}
									>
										[
									</span>
									<span
										className={`absolute right-0 top-2 opacity-0 transition-opacity duration-300 text-cyber-blue ${pathname === link.href ? "opacity-100" : "group-hover:opacity-100"}`}
									>
										]
									</span>
								</Link>
